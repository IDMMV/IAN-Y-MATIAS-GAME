...
window.HK_BUILD_SUPER_SALTO = function(container) {
  ...
  function hurt(sourceX, fall=false) {
    ...
    if (hero.hp<=0) {
      showModal(`
        ...
        <button onclick="window.pl10RetryLevel()" style="margin-top:17px;border:0;border-radius:14px;padding:12px 22px;background:#16a34a;color:#fff;font-weight:900">
          ↺ Reintentar nivel
        </button>
      `);
      sound('lose');
  ...
  window.pl10RetryLevel=()=>startLevel(campaign.level,false);
  window.pl10NextLevel=()=>startLevel(campaign.level,true);
