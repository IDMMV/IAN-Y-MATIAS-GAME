...
window.HK_BUILD_SUPER_SALTO = function(container) {
  ...
  container.innerHTML = `
    <div id="plWorldWrap" class="game-container"
         style="position:relative;width:100%;height:100dvh;min-height:${H}px;overflow:hidden;background:#6ec6ff;border-radius:0;touch-action:none;user-select:none;-webkit-user-select:none">
      ...
      <div id="plControls" style="position:absolute;inset:auto 0 max(10px,env(safe-area-inset-bottom)) 0;z-index:30;
        ...
        <div style="display:flex;gap:10px;pointer-events:auto">
          <button id="plLeft" aria-label="Mover a la izquierda"
                  style="width:72px;height:72px;border-radius:23px;border:3px solid rgba(255,255,255,.85);
                  background:rgba(24,16,28,.86);color:#fff;font-size:2rem;font-weight:900;box-shadow:0 8px 18px rgba(0,0,0,.35);touch-action:none">◀</button>
        ...
        <div style="display:flex;gap:10px;align-items:flex-end;pointer-events:auto">
          <button id="plAction" aria-label="Usar poder"
                  style="width:76px;height:76px;border-radius:24px;border:3px solid rgba(255,255,255,.85);
                  background:linear-gradient(135deg,#f97316,#dc2626);color:#fff;font-size:1rem;font-weight:900;
                  box-shadow:0 8px 18px rgba(0,0,0,.35);touch-action:none">💨<br><span style="font-size:.68rem">IMPULSO</span></button>
          <button id="plJump" aria-label="Saltar"
                  style="width:105px;height:82px;border-radius:25px;border:3px solid rgba(255,255,255,.85);
                  background:linear-gradient(135deg,#2563eb,#16a34a);color:#fff;font-size:1.05rem;font-weight:900;
                  box-shadow:0 8px 18px rgba(0,0,0,.35);touch-action:none">⬆<br>SALTO</button>
        </div>
      </div>
    </div>`;
  ...
  function hurt(sourceX, fall=false) {
    ...
    if(hero.hp<=0){
      ...
      if(fall) showToast(`❤️ Caíste · Quedan ${hero.hp}`);
      else showToast(`❤️ Te golpearon · Quedan ${hero.hp}`);
      respawn();
    }
  }
  ...
  function killEnemy(e, stomp=false) {
    ...
    if(e.hp<=0){
      ...
      if(e.type==='guardian'){
        bossDefeated=true;
        showToast('👹 ¡Guardián derrotado! La puerta final está abierta');
      ...
      showToast(`👹 Guardián: ${e.hp}/${e.maxHp}`);
    }
  ...
  function update() {
    ...
    for(const cp of level.checkpoints){
      if(hero.x>=cp && cp>checkpointX){
        checkpointX=cp;
        showToast('🚩 Punto de control guardado');
      }
    }
    ...
    const goalOpen=levelIndex<2 || bossDefeated;
    if(goalOpen && hero.x+hero.w>level.goalX && hero.x<level.goalX+70){
      completeLevel();
    }
  ...
  function drawGoal() {
    ...
    const open=levelIndex<2||bossDefeated;
    ctx.save();
    if(levelIndex<2){
      ...
      ctx.font='25px Arial';ctx.fillText('⭐',x+29,FLOOR_Y-84);
    }else{
